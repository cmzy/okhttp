...
class AllMainsTest(val className: String) {
  ...
  private fun expectedFailure(
    ...
    return when (className) {
      "mOkhttp3.recipes.CheckHandshake" -> true // by design
      "mOkhttp3.recipes.RequestBodyCompression" -> true // expired token
      else -> false
    }
  }
  ...
  companion object {
    ...
    private fun mainFiles(): List<File> {
      val directories = listOf(
          "$prefix/samples/guide/src/main/java/mOkhttp3/guide",
          "$prefix/samples/guide/src/main/java/mOkhttp3/recipes",
          "$prefix/samples/guide/src/main/java/mOkhttp3/recipes/kt"
      ).map { File(it) }
